subroutine lambda_inner_optimization_decouple_atom(HAMILTONIAN, KINEDEN, &
...
! restrict trial step
        ...
        io_begin
        if (IO%IU0 >= 0) write (IO%IU0, *) "cosine between target and trial: "
        if (IO%IU0 >= 0) write (IO%IU0, *) cos_theta
        io_end
...
end subroutine lambda_inner_optimization_decouple_atom
