...
void timing(mode,utime,stime,now,minpgf,majpgf,maxrsize,avsize,swaps,ios,cswitch,ierr)
...

{
   ...
   *utime    = ((double) usertime.tv_sec) + ((double) usertime.tv_usec) / 1000000.;
   *stime    = ((double) systime.tv_sec ) + ((double) systime.tv_usec ) / 1000000.;
